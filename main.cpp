...
int main()
{
    Mat image = imread("/home/aizat/IG/lena.png", CV_LOAD_IMAGE_GRAYSCALE);
    int histograma[257] = {0};
       ...
       for (int i = 0; i < image.rows; i++)
        ...
        //    histogram[(int)image.at<uchar>(i,j)]++;
            ...
            histograma[Red] = histograma[Red] + 1;
        }

    Mat HistPlotR (355, 600, CV_8UC3, Scalar(0,0,0));
        for (int i = 0; i < 256; i=i+2)
    {
            line(HistPlotR, Point(i, 355), Point(i, 600-histograma[i]), Scalar(0, 0,255),1,8,0);
        }
   int histogram[256];
